  ...
  for (const [gupKey, entry] of Object.entries(LEXICON.verbs)) {
    for (let formIndex = 0; formIndex < entry[mode].length; formIndex++) {
      ...
      for (const tense of tenses) {
        const conjugations = forms[tense] as string[] | undefined;
...
interface MultiWordVerbMatch {
  verbMatch: VerbMatch;
  wordCount: number;
  original: string;
}

function findMultiWordVerbMatch(
  words: string[],
  startIndex: number,
  mode: LanguageMode
): MultiWordVerbMatch | null {
  const maxLen = Math.min(5, words.length - startIndex);
  for (let len = maxLen; len >= 2; len--) {
    const phrase = words.slice(startIndex, startIndex + len).join(" ");
    const normalized = normalizeToken(phrase);
    const match = tryMatchVerb(normalized, mode);
    if (match && !match.entry?.isEmotion) {
      return {
        verbMatch: { ...match, attachedClitic: undefined },
        wordCount: len,
        original: phrase,
      };
    }
  ...
  for (const [verbKey, entry] of Object.entries(LEXICON.verbs)) {
    ...
    for (const form of list) {
      if (normalizeToken(form) === normalized) {
        ...
        return { agentGup: baseGup + "mirri", verbGup: baseGup };
      }
    }
  }
  return null;
...
export function tokenize(text: string, mode: LanguageMode): TokenizerResult {
  ...
  while (i < words.length) {
    const multiVerbMatch = findMultiWordVerbMatch(words, i, mode);
    if (multiVerbMatch) {
      tokens.push({
        original: multiVerbMatch.original,
        normalized: normalizeToken(multiVerbMatch.original),
        type: "verb",
        verbMatch: multiVerbMatch.verbMatch,
        gupKey: multiVerbMatch.verbMatch.gupKey,
      });
      i += multiVerbMatch.wordCount;
      continue;
    }
