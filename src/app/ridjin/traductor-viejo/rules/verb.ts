...
export function detectVerbFeatures(
  ...
  const isPastConjugation =
    ...
    conjugationType === "imperfectSubjunctive";
...
export function applyVerbRules(
  ...
  const effectiveConjugationType =
    treatInfinitiveAsFinite && isInfinitiveForm ? null : conjugationType;
  let verbOptions = determineVerbForms(features, mode, effectiveConjugationType);
  ...
  if (features.mood === "imperative" && features.polarity === "negative") {
    const seen = new Set(results.map((r) => r.gup));
    const miriwForms: { form: VerbFormNumber; formName: VerbFormName }[] = [
      { form: 3, formName: "tertiary" },
      { form: 4, formName: "quaternary" },
    ];

    for (const mForm of miriwForms) {
      const baseGup = getVerbGupForm(verbMatch.gupKey, mForm.form);
      if (!baseGup) continue;
      const gup = `${baseGup}miriw`;
      ...
      results.push({
        ...
        particles: [],
        formUsed: mForm.form,
        formName: mForm.formName,
        explanation: `${config.negImperative} (alt miriw): ${baseGup} + -miriw`,
        features,
        options: verbOptions,
        hasAmbiguity: true,
      });
