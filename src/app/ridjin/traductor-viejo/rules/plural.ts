...
const SUFFIXES_BEFORE_PLURAL: string[] = [
  "walaŋuwuy",
  "kalaŋuwuy",
  "galaŋuwuy",
  "walaŋukurru",
...
function inferPluralBaseAndSuffix(
  gup: string
): { base: string; suffix: string; prefix?: string } | null {
  const trimmed = gup.trim();
  const words = trimmed.split(/\s+/);
  let prefix = "";
  let core = trimmed;
  if (words.length > 1 && (words[0] === "dhuwala" || words[0] === "dhuwali")) {
    prefix = words[0];
    core = words.slice(1).join(" ");
  }

  for (const suffix of SUFFIXES_BEFORE_PLURAL) {
  ...
  const newParts = translation.parts.map((p) => {
    if (explicitPluralParts.includes(p) && canTakeWurru(p.gup)) {
      ...
      if (p.appliedSuffix) {
        const base =
          p.baseGup ||
          (p.gup.endsWith(p.appliedSuffix)
            ? p.gup.slice(0, -p.appliedSuffix.length)
            : p.gup);
        const pluralBase = addWurruPlural(base);
        wurruForm = validarFonologia(pluralBase + p.appliedSuffix);
      } else {
        const inferred = inferPluralBaseAndSuffix(p.gup);
        if (inferred) {
          const pluralBase = addWurruPlural(inferred.base);
          const rebuilt = validarFonologia(pluralBase + inferred.suffix);
          wurruForm = inferred.prefix ? `${inferred.prefix} ${rebuilt}` : rebuilt;
        } else {
